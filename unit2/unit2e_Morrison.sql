...
SELECT state, COUNT(full_name) AS count
from teams
GROUP BY state
ORDER BY count DESC
...
-- California = 4, Texas = 3

-- In query 6 you used both WHERE and HAVING. Explain in your own
-- words what each one filtered.
-- Having was the only one i used in query 6. It filtered the teams that had more than 200 wins.
...
-- GROUP BY:
groups all similar values in a column together

-- HAVING:
filters groups after they have been created by GROUP BY
